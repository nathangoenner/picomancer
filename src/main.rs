...
fn main() -> ! {
    ...
    const START_INDEX_MAX:i32 = (STRIP_LEN as i32);

    let mut loop_cnt:u32 = 0;
    let mut loop_cnt_mod:u32 = 20;
    ...
    loop {
        loop_cnt += 1;
        if 0 == (loop_cnt % loop_cnt_mod) {
            start_index += 1;
            if start_index >= START_INDEX_MAX {
                start_index = 0; 
            }
        }
        for (i, led) in leds.iter_mut().enumerate() {
            ...
            let sat = 1.0;
            let len = STRIP_LEN as i32;
            let mut dist = start_index - (i as i32);
            if dist < 0 {
                dist = len + dist;
            }
            let val = libm::powf(2.0, -(dist as f32));
        ...
        if button_pin.is_low().unwrap() {
            ...
            //     button_state = true;
            //     loop_cnt_mod = match loop_cnt_mod {
            //         10 => 20,
            //         20 => 40,
            //         40 => 60,
            //         60 => 100,
